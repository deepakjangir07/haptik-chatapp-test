...
def create_group(request):
    ...
    if request.method == "POST":
        ...
        connection.users.add(current_user.id)


        if request.POST.getlist('users'):
            connection = Connection.objects.get(id=connection.id)
            for user_id in request.POST.getlist('users'):
                connection.users.add(user_id)
                connection.save()
        new_group = Group.objects.last()        
        User.objects.create(username= new_group.title,is_active=False)

    return render(request, 'core/create_group.html', {'current_user': current_user,'all_users':all_users, })
